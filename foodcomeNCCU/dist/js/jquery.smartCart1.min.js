$(document).ready(function () {
    ...
    db.collection(name).get().then(function (querySnapshot) {
        ...
        const day = order.map(item => Object.values(item)[0]);
        const smart = order.map(item => Object.values(item)[4]);

        const storename = order.map(item => Object.values(item)[5]);
        const totoprice = order.map(item => Object.values(item)[6]);
        ...
        no.forEach(function (element) {
            ...
            $('button[name=' + element + ']').click(function () {
                ...
                smart[element] = smart[element].replace('<button class="btn btn-info sc-cart-checkout" id="btn btn-info sc-cart-checkout" type="button">Checkout</button> <button class="btn btn-danger sc-cart-clear" id="btn btn-danger sc-cart-clear" type="button">Clear</button></div></div></div>', "")
                smart[element] = smart[element].replace('<button type="button" class="sc-cart-remove" id="sc-cart-remove">×</button>', "")

                console.log('sm', smart[element] + 'sm')
                $('aside.col-md-4').html("'" + smart[element] + "'")
